...
if __name__ == '__main__':
    ...
    checkpoint = tf.train.Checkpoint(myAwesomeModel=model)
    checkpoint.restore(tf.train.latest_checkpoint('/root/shy/train_imagenet/checkpoints_hash_imagenet/image_512bit/VGG19/0.605'))
    ...
    for epoch in range(c.epoch_num):
        ...
        if test_acc > 0:
            name = "./checkpoints_hash_imagenet/image_512bit_418/VGG19/" + str(np.round(test_acc.numpy(), 3)) + '/model.ckpt'
            if not os.path.exists("./checkpoints_hash_imagenet/image_512bit_418/VGG19/" + str(np.round(test_acc.numpy(), 3))):
                os.makedirs("./checkpoints_hash_imagenet/image_512bit_418/VGG19/" + str(np.round(test_acc.numpy(), 3)))
