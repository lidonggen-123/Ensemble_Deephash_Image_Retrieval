...
def load_image(image_path, label, augment=False, crop_10=False):
    ...
    label_one_hot = np.zeros(c.num_class)
    label_one_hot[label] = 1.0

    return image, label_one_hot


def train_iterator(list_path=c.imagenet_train_path):
...
def test_iterator(list_path=c.imagenet_test_path):
...
def test_10_crop_iterator(list_path=c.sun397_test_path):
    images, labels = load_list(list_path, c.test_data_path)
    dataset = tf.data.Dataset.from_tensor_slices((images, labels))
    dataset = dataset.map(
