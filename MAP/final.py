...
def list_add(K, pic_path1, pic_path2, pic_path3, pic_path4, hanming_matrix1, hanming_matrix2, hanming_matrix3,hanming_matrix4):
    list1_1, list1_2, list1_3, list1_4= [], [], [],[]
    # 三个for循环比较慢
    # 键 1-n   值:正确图片的索引
    ...
    for k, v in pic_path3.items():
        ...
        if count==1000:
          count=0
          break
    for k, v in pic_path4.items():
        result4 = hanming_matrix4[pic_path4[k]]
        list1_4.append(result4)
        count+=1
        if count==1000:
          count=0
          break
    ave=(np.mean(list1_1)+np.mean(list1_2)+np.mean(list1_3)+np.mean(list1_4))/4
    ...
    a4=np.linspace(np.mean(list1_4)-ave,np.mean(list1_4)-ave,1000)
    b1=np.linspace(acc[0],acc[0],1000)
    b2=np.linspace(acc[1],acc[1],1000)
    b3=np.linspace(acc[2],acc[2],1000)
    b4=np.linspace(acc[3],acc[3],1000)
    
    list1_1=(list1_1-a1)*b1
    list1_2=(list1_2-a2)*b2
    list1_3=(list1_1-a3)*b3
    list1_4=(list1_1-a4)*b4
    new_list=[]
    path = list(pic_path1.keys())[:K] + list(pic_path2.keys())[:K] + list(pic_path3.keys())[:K]+list(pic_path4.keys())[:K]
    new_list=np.concatenate([list1_1,list1_2,list1_3,list1_4])
...
def precision(train_label, train_binary, train_base, test_label, test_binary, test_query, top_k):
 ...
 8318, 8383, 8390, 8675]
    ...
    for i in li:
        ...
        query_binary_4 = test_binary[3][i]
        ...
        similarity_4 = hanming(train_binary[3], query_binary_4)
        ...
        buffer_yes_4 = fun(train_label[3], y2_4, K, test_label[3][i], buffer_yes_4)

        P_1 = true_divide(buffer_yes_1, Ns)
        P_2 = true_divide(buffer_yes_2, Ns)
        P_3 = true_divide(buffer_yes_3, Ns)
        P_4 = true_divide(buffer_yes_4, Ns)
        ...
        if sum(buffer_yes_4) == 0:
            AP_4[count1] = 0
        else:
            AP_4[count1] = sum(np.array(P_4) * np.array(buffer_yes_4)) / sum(buffer_yes_4)
        ...
        order_map_4 = [similarity_4[i] for i in y2_4]
        ...
        hanming_normalazition4 = hanming_normalazition(order_map_4)
        ...
        hanming_matrix4 = [i  for i in hanming_normalazition4]
        path, new_list = list_add(K, pic_path1, pic_path2, pic_path3,pic_path4,  hanming_matrix1, hanming_matrix2, hanming_matrix3,hanming_matrix4)
    ...
    return map_1, map_2, map_3,  map_4,ap1


if __name__ == '__main__':
    # vgg resnet dense
    acc = [0.764, 0.735, 0.753,0.759]
    print("111")
    ...
    print("555")
    TestData_Desnet1 = np.load("/home/ubuntu/lxd-workplace/shy/hashcode_imagenet_1024bit/NPY/imagenet/1024bit/Resnet_npy/CheckData/Code_Check.npy", allow_pickle=True)
    TestLabel_Desnet1 = np.load("/home/ubuntu/lxd-workplace/shy/hashcode_imagenet_1024bit/NPY/imagenet/1024bit/Resnet_npy/CheckData/Code_label.npy", allow_pickle=True)
    TestPath_Desnet1 = np.load("/home/ubuntu/lxd-workplace/shy/hashcode_imagenet_1024bit/NPY/imagenet/1024bit/Resnet_npy/CheckData/Code_Path.npy", allow_pickle=True)
    print("666")
    DataBase_Desnet1 = np.load("/home/ubuntu/lxd-workplace/shy/hashcode_imagenet_1024bit/NPY/imagenet/1024bit/Resnet_npy/DataBase/Code_Base.npy", allow_pickle=True)
    DataLabel_Desnet1= np.load("/home/ubuntu/lxd-workplace/shy/hashcode_imagenet_1024bit/NPY/imagenet/1024bit/Resnet_npy/DataBase/Code_number.npy", allow_pickle=True)
    DataPath_Desnet1 = np.load("/home/ubuntu/lxd-workplace/shy/hashcode_imagenet_1024bit/NPY/imagenet/1024bit/Resnet_npy/DataBase/Code_Pic.npy", allow_pickle=True)
    print("777")
    DataLabel = [DataLabel_VGG, DataLabel_Resnet, DataLabel_Desnet,DataLabel_Desnet1]
    DataBase = [DataBase_VGG, DataBase_Resnet, DataBase_Desnet,DataBase_Desnet1]
    DataPath = [DataPath_VGG, DataPath_Resnet, DataPath_Desnet, DataPath_Desnet1]

    TestLabel = [TestLabel_VGG, TestLabel_Resnet, TestLabel_Desnet, TestLabel_Desnet1]
    TestData = [TestData_VGG, TestData_Resnet, TestData_Desnet, TestData_Desnet1]
    TestPath = [TestPath_VGG, TestPath_Resnet, TestPath_Desnet, TestPath_Desnet1]
