...
log_dir = 'logs/Resnet/' + current_time
...
if __name__ == '__main__':
    ...
    for epoch in range(epoch_num):
        ...
        if test_acc.numpy() > 0.85:
            name = "./ckeckpoints_hash_cifar10/32bit/Resnet/" + str(np.round(test_acc.numpy(), 3)) + '/model.ckpt'
            if not os.path.exists("./ckeckpoints_hash_cifar10/32bit/Resnet/" + str(np.round(test_acc.numpy(), 3))):
                os.makedirs("./ckeckpoints_hash_cifar10/32bit/Resnet/" + str(np.round(test_acc.numpy(), 3)))
