...
import datetime
from dense_model import my_densenet
import config as c
weight_decay = 1e-4
...
log_dir = 'logs/Densenet-cos/' + current_time
...
if __name__ == '__main__':
    ...
    for epoch in range(epoch_num):
        ...
        if test_acc.numpy() > 0.9:
            name = "./ckeckpoints_hash_cifar10/32bit/Densenet/" + str(np.round(test_acc.numpy(), 3)) + '/model.ckpt'
            if not os.path.exists("./ckeckpoints_hash_cifar10/32bit/Densenet/" + str(np.round(test_acc.numpy(), 3))):
                os.makedirs("./ckeckpoints_hash_cifar10/32bit/Densenet/" + str(np.round(test_acc.numpy(), 3)))
