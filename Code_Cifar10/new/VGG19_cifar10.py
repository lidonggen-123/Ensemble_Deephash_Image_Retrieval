...
if __name__ == '__main__':
    ...
    for epoch in range(epoch_num):
        ...
        if test_acc.numpy() > 0.85:
            name = "./ckeckpoints_hash_cifar10/12bit/VGG19/" + str(np.round(test_acc.numpy(), 3)) + '/model.ckpt'
            if not os.path.exists("./ckeckpoints_hash_cifar10/12bit/VGG19/" + str(np.round(test_acc.numpy(), 3))):
                os.makedirs("./ckeckpoints_hash_cifar10/12bit/VGG19/" + str(np.round(test_acc.numpy(), 3)))
