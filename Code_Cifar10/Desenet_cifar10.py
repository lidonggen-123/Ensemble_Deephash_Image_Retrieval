...
log_dir = 'logs/' + current_time + "-Desenet-cifar10"
...
def train(model, optimizer, images, labels):
    ...
    return sum_loss / iterations_per_epoch, l2_loss(
        model), sum_hash_loss / iterations_per_epoch, sum_accuracy / iterations_per_epoch
...
if __name__ == '__main__':
    ...
    for epoch in range(epoch_num):
        ...
        if test_acc.numpy() > 0.80:
            name = "./ckeckpoints_hash_cifar10/48bit/Densenet/" + str(np.round(test_acc.numpy(), 3)) + '/model.ckpt'
            if not os.path.exists("./ckeckpoints_hash_cifar10/48bit/Densenet/" + str(np.round(test_acc.numpy(), 3))):
                os.makedirs("./ckeckpoints_hash_cifar10/48bit/Densenet/" + str(np.round(test_acc.numpy(), 3)))
