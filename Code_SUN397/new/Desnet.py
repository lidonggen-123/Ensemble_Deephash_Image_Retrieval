...
from Mynet import Resnet_Model, DenseNet_Model
...
def train(model, optimizer, train_iterator, log_file):
    ...
    #for i in range(c.iterations_per_epoch):
...
if __name__ == '__main__':
    ...
    for epoch in range(c.epoch_num):        
        ...
        if test_acc > 0.5:
          name = "./checkpoints_hash_sun397/1024bit/Dense-424/" + str(np.round(test_acc.numpy(), 3)) + '/model.ckpt'
          if not os.path.exists("./checkpoints_hash_sun397/1024bit/Dense-424/" + str(np.round(test_acc.numpy(), 3))):
            os.makedirs("./checkpoints_hash_sun397/1024bit/Dense-424/" + str(np.round(test_acc.numpy(), 3)))
