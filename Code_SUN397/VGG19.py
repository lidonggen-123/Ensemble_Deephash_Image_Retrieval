...
if __name__ == '__main__':
    ...
    for epoch in range(c.epoch_num):
        ...
        if test_acc > 0:
            name = "./checkpoints_hash_sun397/48bit/VGG19/" + str(np.round(test_acc.numpy(), 3)) + '/model.ckpt'
            if not os.path.exists("./checkpoints_hash_sun397/48bit/VGG19/" + str(np.round(test_acc.numpy(), 3))):
                os.makedirs("./checkpoints_hash_sun397/48bit/VGG19/" + str(np.round(test_acc.numpy(), 3)))
